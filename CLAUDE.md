...
- `docs/oauth/oauth-integration-guide.md` — infra's generated mirror, never edited here at all.
...
One contract doc is vendored here: `docs/oauth/oauth-integration-guide.md`, a **read-only mirror** of infra's `docs/integration/oauth/` (GENERATED banner), regenerated by nextmoe-docs's `pnpm docs:sync`. Every other contract doc — the rest of OAuth, image service, artifact, catalog, the galgame-wiki tombstone — is read at its infra source, `${CODEX_SESSION_REPO%/*}/nextmoe-infra/docs/`, and never copied here.
**To change a contract, change it at the infra source.** Core invariants:
...
Contract docs live in infra: `docs/integration/oauth/`, `docs/image_service/`, `docs/artifact/`, `docs/catalog/`, and the galgame retirement tombstone.

## This Repo's Key Points

- Auth lives in `internal/middleware/auth.go`: the web uses a **BFF opaque session** (`kungal_session` cookie + the OAuth token stored in Redis) with **90-day sliding renewal** — the model and the 2026-06 fix are in `refs/docs/proj/session-lifetime.md`. The first-party Flutter App instead sends `Authorization: Bearer <OP access token>`, verified locally against the OP's JWKS and gated by the `KUN_BEARER_CLIENT_IDS` allow-list (`middleware/bearer.go`). **A Bearer request never holds staff powers**: check capabilities with `user.Can` / `user.CanModerate`, never `perm.CanUser` / `role.Can*` on `user.Roles` (`bearer_guard_test.go` enforces this). The App contract (Bearer, `Idempotency-Key`, `/api/v1/app/version`) is in `refs/docs/proj/app-direct-api.md`. **Neither a Bearer request nor a web session carries a content preference into v1 reads**: the result set is chosen by the explicit `include_nsfw` query param (`refs/docs/proj/api-v1/01-standard.md` §3). `X-Kungal-Nsfw` was deleted 2026-09-23 and the per-request stance pipeline (`IsSFW`, `middleware/bearer_stance.go`) on 2026-09-24 (#248); `GET /api/v1/me/account` returns `content_stance: null` for a Bearer request.
- **Docs are local and internal.** Since 2026-09-24 the forum's design docs (`proj/`, the api-v1 standard, wave contracts, session SOP and CHANGELOG, runbooks) live in `refs/docs/`, which is gitignored: published docs drifted, so we keep them for ourselves. Tracked `docs/` holds only the localized READMEs (`docs/readme/`) and the OAuth guide mirror. A worktree has no `refs/`; read it in the source checkout (`$CODEX_SESSION_REFS` when present). A tracked file may name a `refs/docs/...` path, but nothing that is built, tested or deployed may depend on one.
- Editing `docs/oauth/oauth-integration-guide.md` is **wrong** — it is infra's mirror; change infra, then run `docs:sync` in nextmoe-docs.
