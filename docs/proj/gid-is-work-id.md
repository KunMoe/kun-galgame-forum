...
- ~~**收藏条目的写入方**~~ 已在窗口里处理（用户把这一项委托给 kun-galgame-forum-37 会话按其建议执行）：
  - 导入期条目按收藏夹与创建时间精确配对：199,801 条配上，改写 147,200 条。
  - 9-07 之后配不上的 22,164 条按归属判定：20,130 条判为论坛写入，按改号表改写；其中 18,312 条凭论坛的收藏通知消息判定。另外 2,034 条判为 moyu 写入，保持原样。
  - 抽样对照 catalog 当天的请求日志（每次 PUT 的来源 IP）：论坛 548/549 正确，moyu 10/10 正确。
  - 同一收藏夹内去重 79 条，保留最早的一条；重算了 7,285 个收藏夹的 `item_count`。
  - 脚本与输入在 `kungal-neo:~/g0/sql/02_catalog_folders.sql` 与 `~/g0/*.tsv`，窗口前的表备份在 `~/g0/backup/kun_catalog-g0-tables.dump`。
  - G 会话事后复核：272,769 条收藏里没有指向不存在作品的，没有同夹重复，`item_count` 零漂移。
