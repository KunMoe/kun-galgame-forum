...
- 参数：`cursor`、`limit`（1–50，默认 20）、`reason`（可选）。
  ...
  - `reason`：开放词表（OAuth 会加新 reason），`pattern ^[a-z][a-z0-9_]{0,39}$`。形状非法 → `400 INVALID_PARAMETER`（huma 对 query 参数的校验失败在本仓映射成 400，与 `page_test.go` 一致）；形状合法但上游不认识的 reason **不是错误**：OAuth 的 `respondLog` 只按它过滤，结果就是空页（浏览器验收时读 infra 源码核实；初稿写的「上游回 400 → 我们回 400」是凭空推测的路径，实现照着写了，在开发环境里把上游的 500 说成了「reason 不认识」）。上游任何失败一律 `503`。
...
| `PATCH /me/profile` 响应的 `id` 取自上游 `/auth/me` | 取自会话 | 上游只有 `uuid`，没有数字 `id`；假上游带了 `id`，所以测试绿、真实环境发出 `"id": "0"`。假上游已改成真实形状 |
| 流水的上游错误码映射成 `400 INVALID_PARAMETER` | 一律 `503` | 见 §3.3 |
