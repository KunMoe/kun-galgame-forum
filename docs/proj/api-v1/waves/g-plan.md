...
- **G4**（5）：`GET /galgame/:id`、`PUT /galgame/:id/like`、`GET /galgame/:id/link/all`、`GET /galgame/interactions/mine`、`GET /galgame/drafts`（删，无替代）
...
- **G7**（22）：`DELETE /galgame/:id`（投稿撤回，`GalgameSubmissionHandler.Withdraw`；2026-09-24 从 G4 改归 G7）、`POST /galgame/submit`、`GET /galgame/search/wizard`、`GET /galgame/mine`、`GET /galgame/audited`、`POST /galgame/:id/resubmit`、`DELETE /galgame/:id/draft`、`GET /galgame/:id/edit/{bootstrap,diff,revisions}`、`GET` / `POST /galgame/:id/edit/proposals`、`POST /galgame/:id/edit/revert`、`GET /galgame-edit/{mine,queue}`、`GET /galgame-edit/proposals/:id`、`POST /galgame-edit/proposals/:id/{amend,decline,merge,withdraw}`、`GET /admin/galgame/submissions`、`POST /admin/galgame/:id/review`
...
- **G4.1 的教训，G5 起每段契约都要做**：本段从 catalog 收窄的每个封闭词表与长度上限，都对 infra 的定义（`nextmoe-infra/apps/api/internal/platform/apiv2/repr/*.go`、`collect/*.go`）逐个比，不对 dev 数据恰好有的值比；每个有意丢行的分支打 WARN。G4 只收三个 `roster_role` 值，上线后丢了 5% 的角色。
