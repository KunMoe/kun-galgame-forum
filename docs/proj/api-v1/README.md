...
|---|---|---|---|---|
| U | 用户 `/user/**` + 删号管理面：~~**U1**「我」的面 12 条~~ ✅ → ~~**U2** 公开资料、名片、通知偏好 4 条~~ ✅ → ~~**U3a** 话题 / 回复 / 评论 3 条~~ ✅ → ~~**U3b** galgames / galgame-comments / resources / ratings 4 条~~ ✅ → **U3c** `PurgeHandler` 的 `/admin/user/:id/content(-stats)` 2 条 + 清空存档（破坏性，单独一段，[契约](waves/u3c-purge.md)，迁移 120）。`/user/:id/toolsets` 与 `/collections` 归 G 轨（同一子集合形状） | 2 | 120–129 | 已认领（分支 `api-v1/u3c-purge`） |
