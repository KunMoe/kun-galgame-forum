...
| U3c | 管理员清空用户内容：2 条旧路由 → `GET` / `DELETE /api/v1/admin/user-contents/{user_id}`；清空存档（迁移 120，触发器捕获清空事务改动的每一行，30 天，`user_purge_restore`）；U3c.1 加 `cmd/purge-restore` 连 community 一起撤销（[契约](waves/u3c-purge.md)，PR #236） | ✅ 2026-09-24 |
...
|---|---|---|---|---|
| U | 用户 `/user/**` + 删号管理面：~~**U1**「我」的面 12 条~~ ✅ → ~~**U2** 公开资料、名片、通知偏好 4 条~~ ✅ → ~~**U3a** 话题 / 回复 / 评论 3 条~~ ✅ → ~~**U3b** galgames / galgame-comments / resources / ratings 4 条~~ ✅ → ~~**U3c** `PurgeHandler` 的 `/admin/user/:id/content(-stats)` 2 条 + 清空存档（[契约](waves/u3c-purge.md)，迁移 120，#236）~~ ✅。`/user/:id/toolsets` 与 `/collections` 归 G 轨（同一子集合形状） | 0 | 120–129 | ✅ U 轨完成（120 已用） |
| G | galgame 主域 + `-edit` + `-quiz` + `-resource` + `toolset` + 各自的 admin/user 面，含 `/user/:id/toolsets` 与 `/user/:id/collections`（**不含** `/galgame/:id/comments*` 与 `/galgame/comments/*`，已归 RC） | 69 | 140–159 | ✅ 2026-09-24 完成。~~**G0** 改号：论坛 galgame id ≡ catalog work id~~ ✅ 2026-09-23（G0a #188，G0b #192，迁移 140/141，[计划](../gid-is-work-id.md)、[操作单](../g0-window-runbook.md)）→ ~~**G1** 工具集 16 条~~ ✅ 2026-09-23（#215，[契约](waves/g1-toolsets.md)，迁移 142；G1.1 迁移 144）→ ~~**G2** 题库 13 条~~ ✅ 2026-09-24（#219，[契约](waves/g2-quizzes.md)，迁移 143）→ ~~**G3** galgame 资源 12 条~~ ✅ 2026-09-24（#223，[契约](waves/g3-resources.md)，迁移 145）→ ~~**G4–G6** 作品详情、浏览、catalog 用户面~~ ✅ 2026-09-24（G4 #225/#226、G5 #227、G6 #231/#232/#233，无迁移）→ ~~**G7** 投稿、认领审核、资料编辑引擎 22 条~~ ✅ 2026-09-24（G7b 编辑引擎 13 条 #235、G7a 投稿与认领审核 9 条 #237，[契约](waves/g7-contribution.md)，无迁移）。逐段见 [G 轨计划](waves/g-plan.md) |
| X | 零散：~~**X2**：`/search` 3、`/ranking` 3、`/community` 3、`/auth/me` 1、`/activity` 3、`/admin` 总览 2、`/home` 1~~ ✅（`/search` 另 3 条资源 / 资料库车道等 G、GE 定形；`/auth` 另 2 条是 BFF 登录管道，永留 v1 外）；~~**X1**：`/friend-link`(+admin) 5、`/app` 1（X1a #202）、`/category`+`/section` 2（X1b #205）、`/news` 4 + `/rss/topic` 1（X1c #212）、`/image` 4（X1d #216，另：全站过大请求体 413）~~ ✅（`/rss/galgame` 归 G 的浏览集合）。~~**后续**：不可渲染作品的详情页回 HTTP 200 +「未找到这个 Galgame」（软 404），另开小网页 PR~~ ✅ #230 改成真 404 | 39 | 195–209 | X1、X2 完成。195 已用于 galgame 镜像漂移（[文档](../galgame-mirror-drift.md)） |
