...
| G | galgame 主域 + `-edit` + `-quiz` + `-resource` + `toolset` + 各自的 admin/user 面，含 `/user/:id/toolsets` 与 `/user/:id/collections`（**不含** `/galgame/:id/comments*` 与 `/galgame/comments/*`，已归 RC） | 69 | 140–159 | 已认领（分支 `api-v1/g-galgame`）。~~**G0** 改号：论坛 galgame id ≡ catalog work id~~ ✅ 2026-09-23（G0a #188，G0b #192，迁移 140/141，[计划](../gid-is-work-id.md)、[操作单](../g0-window-runbook.md)）→ ~~**G1** 工具集 16 条~~ ✅ 2026-09-23（#215，[契约](waves/g1-toolsets.md)，迁移 142；G1.1 迁移 144）→ **G2** 题库 13 条（[契约](waves/g2-quizzes.md)，迁移 143）→ G3 起见 [G 轨计划](waves/g-plan.md) |
