...
| X | 零散：~~**X2**：`/search` 3、`/ranking` 3、`/community` 3、`/auth/me` 1、`/activity` 3、`/admin` 总览 2、`/home` 1~~ ✅（`/search` 另 3 条资源 / 资料库车道等 G、GE 定形；`/auth` 另 2 条是 BFF 登录管道，永留 v1 外）；~~**X1**：`/friend-link`(+admin) 5、`/app` 1（X1a #202）、`/category`+`/section` 2（X1b #205）、`/news` 4 + `/rss/topic` 1（X1c #212）、`/image` 4（X1d #216，另：全站过大请求体 413）~~ ✅（`/rss/galgame` 归 G 的浏览集合）。**后续**：不可渲染作品的详情页回 HTTP 200 +「未找到这个 Galgame」（软 404），另开小网页 PR | 39 | 195–209 | X1、X2 完成。195 已用于 galgame 镜像漂移（[文档](../galgame-mirror-drift.md)） |
