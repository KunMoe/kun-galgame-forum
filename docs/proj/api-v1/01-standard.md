...
| 语言标签（工具集 `language`；资源 `languages` / `resource_language` / `resource_languages`） | `zh-cn` | 小写 BCP 47 标签：存储、写路径与网页词表都用它，WS 的 `language` 已按同样的标签下发；改成 `zh_cn` 就要在每次读写上永久翻译。F1 对这几个属性改查「小写 BCP 47 或 `other` / `others`」 |
