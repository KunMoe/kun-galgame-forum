...
| 2026-09-27 | `fb-push`: the activity pusher (migration 198 outbox + trigger + backfill, drainer with read-time revisions, 422 bisect, nightly field-by-field reconcile), 1.5k lines incl. tests | grok-4.6-xhigh | 1326 s, 361 calls, 395k in + 16M cache read, 101k out | Every adjudicated rule landed and 10 DB tests passed first time on a real database; its §2 list was accurate and caught **a flaw in the orchestrator's own design**: the book said a live item with no title is held like an upstream failure, so one row with a permanently missing name (or one assemble error that is deterministic, such as a resource type outside the vocabulary) would have wedged the queue and the whole backfill forever. It also found a short `results` slice acked unpushed rows, and it flagged — but coded around — work names read as the raw `display_name` instead of the forum's zh-first rule. It reported the infra contract as missing the write faces because it read infra's **stale local checkout**, not `origin/main`; give executors the extracted file, not the checkout. Acceptance: per-row assemble fallback (a row that fails alone goes to the back of the queue), title falls back to the object label, results-length check, reconcile tombstones hidden rows itself and skips failed rows instead of aborting the night, zh-first work names, an index on `enqueued`; 3 new DB tests; 10/10 orchestrator mutants died |
| 2026-09-27 | `fb-read`: the 关注 faces moved onto community's grouped feed + a public group-items op, phase-1 local code deleted | grok-4.6-xhigh | ~1300 s, 387 calls, 524k in + 13M cache read, 90k out | Clean: resolved four G8 clashes by renaming (`calendar_date`, `actor`, `in_site_path`, `related_work_id`) rather than allow-listing, kept `FeedRow` intact for the parallel pusher, and edited one file outside its writable paths only because the build required it (reported). Same stale-infra-checkout false alarm as `fb-push`. All 23 DB tests passed first time; 10/10 orchestrator mutants died |
| 2026-09-27 | `fb-notify`: follow notify levels (PATCH), kind-10 mirror with retraction, local followee fan-out removed | grok-4.6-xhigh | 1296 s, 362 calls, 149k in + 2.3M cache read, 25k out | Correct and well reported (G8 → `notify_level` on the response, F8 forced English doc text, a stale kind-9 path noted). The sandbox refused its file delete, so it left `write_followee.go` as an empty `package` file; it also left a dead `community` field on `Writes`, a comment that restated the SQL, and a test whose name still promised the removed behaviour. All DB tests passed first time; 7/7 mutants died (one rerun after a build-error "kill") |
